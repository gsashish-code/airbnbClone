...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<?>> handleResourceNotFoundException(ResourceNotFoundException error){
       ApiError apiError=ApiError.builder().status(HttpStatus.NOT_FOUND).message(error.getMessage()).build();
        return buildApiResponse(apiError);
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ApiResponse<?>> handleResourceNotFoundException(Exception error){
        ApiError apiError=ApiError.builder().status(HttpStatus.INTERNAL_SERVER_ERROR).message(error.getMessage()).build();
        return buildApiResponse(apiError);
    }
